...
*/
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) { //画点函数
  #ifdef WIN32
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
        LCD_TFT_PutPixel(xPhys, yPhys, PixelIndex);//画点函数
    }
...
*   that no check on the parameters needs to be performed.
*/
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) { //读点函数
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      // TBD by customer...
      //
        PixelIndex = LCD_TFT_GetPixel(xPhys, yPhys);//读点
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
